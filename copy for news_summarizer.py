    ...
    if cat_op == category[0]:
        ...
        with st.expander("PLEASE READ! : What is Top News?"):
            st.write("""
                ...
                - Top News are recenT, relevant news in the Philippines gathered from different sources!
...
def display_news(list_of_news, news_quantity, stop_words, bart_tokenizer, bart_model, category):
    for c, news in enumerate(list_of_news[:news_quantity], start=1):
        ...
        with st.expander(news.title.text):
            ...
            if category in st.session_state["history"]:
                st.session_state["history"][category].append({"title": news.title.text, "summary": bart_summary})
...
def run():
    ...
    if st.sidebar.checkbox("View History"):
        st.subheader("Article History")
        for category, history in st.session_state["history"].items():
            if history:
                st.write(f"**{category}**")
                for item in history:
                ...
                st.write(f"**{category}:** No articles viewed yet.")
...
def run():
    ...
    history_expander = st.sidebar.expander("History", expanded=True)

    # Show history for the selected category
    if cat_op != category[0]:  # Don't show history if no category is selected
        with history_expander:
            st.write(f"**History for {cat_op}:**")
            
            # Get the history for the selected category
            history_data = st.session_state["history"].get(cat_op, [])
            
            if history_data:
                for i, article in enumerate(history_data[-5:], 1):  # Display the last 5 articles
...
def run():
    ...
    for cat in categories:
        with st.sidebar.expander(f"History: {cat}", expanded=True):
...
#With working prototype history
